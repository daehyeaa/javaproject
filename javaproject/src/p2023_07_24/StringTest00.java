...
class StringTest00 {
	public static void main(String[] args) {
		// String 클래스 만이 new String 으로 객체 생성을 할 수 있곡 직접 값을 적을 수 있다.
		
		// 자신의 클래스에 없는 메소드는 부모의 클래스의 필드나 메소드 사용이 가능하다.
		
		String str1 = new String("Java Programming");	//String(String original) 생성자를 사용하는 것이다. 라이브러리에서 제공하는 생성자,메소드만 쓸 수 있다.
		...
		String str3 = "Java Programming"; // str3 str4 주소값이 서로 값다
		String str4 = "Java Programming"; // 주소값과 실제값을 비교하는 2가지를 할 때도 있다 주소값에 대한 비교 실제값에 대한 비교
		...
		if (str1.equals(str2)) { // 값비교
			System.out.println("같다같다");
		} else {
			System.out.println("다르다다르다");
		}
...
//	String, StringBuffer, StringBuilder.. 모두 문자열을 저장하고, 관리하는 클래스입니다.  굳이 여러가지를 만들어놓은 이유는 무엇일까요.
//	1) String
...
//
//	사실 JDK 1.5버전 이전에서는 문자열연산(+, concat)을 할때에는 조합된 문자열을 새로운 메모리에 할당하여 참조함으로 인해서 성능상의 이슈가 있었습니다. 
//	그러나 JDK1.5 버전 이후에는 컴파일 단계에서 String 객체를 사용하더라도 StringBuilder로 컴파일 되도록 변경되었습니다. 그리하여 JDK 1.5 이후 버전에서는 String 클래스를 활용해도 StringBuilder와 성능상으로 차이가 없어졌습니다.
